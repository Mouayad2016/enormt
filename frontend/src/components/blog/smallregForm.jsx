...
const validateForm = () => {
  ...
  return (
    <div className={`mt-sm-4 bg-light p-3 rounded-custom ${hideOnMobile === true ? 'hide-on-mobile' : 'hide-on-desc'}`}>
      ...
      <form id='email-form2' name='email-form' className='subscribe-form'>
        <div className="field-container">
          <div className='smreg d-md-flex d-flex'>
            ...
            {loading === true ? (
              <Box sx={{ height: "50px" }}>
                <CircularProgress className="btn btn-omaina-color  mt-4" size={15} />
              </Box>
            ) : (
              <Box sx={{ height: "50px" }}>
              <button type="submit" className="btn btn-soft-primary" onClick={handleSubmit} required>Skicka</button>
              </Box>
            )}
          </div>
